...
## Comparing rarefaction-based and probabilistic uncertainty

So far, all of the uncertainty explored in this vignette comes from repeated rarefaction: for a fixed sequencing depth, we repeatedly sub-sample each sample's reads and observe how much the resulting ordination position moves around.

**Sibyl** also provides an alternative way of representing compositional uncertainty, based on Monte Carlo sampling from the Dirichlet posterior distribution estimated by `ALDEx2::aldex.clr()`, implemented in `dirichlet_ordination()`. Instead of repeatedly sub-sampling reads, each Monte Carlo draw represents one plausible compositional state of the observed count data given its Dirichlet posterior. These replicate compositions are ordinated using Aitchison distances, aligned by Procrustes transformation, and summarized with consensus coordinates and uncertainty clouds, using the same downstream pipeline as `repeated_rarefaction()`.

This makes it possible to directly compare the two sources of uncertainty on the same dataset: stochastic sub-sampling (rarefaction) versus probabilistic compositional modelling (Dirichlet).
...
Both panels show the same samples aligned by Procrustes rotation, with consensus (median) coordinates highlighted in red. Since the two workflows use different distance measures (Bray-Curtis for rarefaction, Aitchison for the Dirichlet approach) and different sources of stochasticity, the absolute scale and orientation of the two ordinations are not directly comparable point-by-point. What is comparable is the relative spread of each sample's cloud of repeats/draws: samples with lower sequencing depth tend to show a wider cloud under repeated rarefaction, since low depth means each sub-sample discards proportionally more of the observed community. The Dirichlet-based approach captures a related but distinct notion of uncertainty, arising from how confidently the observed counts pin down the underlying proportions, rather than from the act of discarding reads.

Using both approaches side by side gives a more complete picture of how much of the variability observed in an ordination is attributable to the chosen sub-sampling scheme, versus how much reflects genuine uncertainty in the compositional estimate itself.
